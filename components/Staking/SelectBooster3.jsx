...
const SelectBooster3 = ({addBooster3, setOpenBoosterPopUp3}) => {
  ...
  return (
    <div className="">
      ...
      <div onClick={onStakeBooster} className="bg-secondary cursor-pointer  p-1 rounded-lg text-black px-4">
          <h1>Stake {selectBooster.name} <span className="ml-4 text-white">+12%</span></h1>
      </div>
        }
      </div>
        ...
        {WalletNfts.map((nft) => (
          ...
          >
            ...
            <Image
              src={nft.nft}
              alt="nft"
              className={select ? "w-32 cursor-pointer hover:opacity-70 border border-1 transition ease-in-out duration-200" : "w-32 cursor-pointer hover:opacity-70 transition ease-in-out duration-200"}
            />
            </label>
